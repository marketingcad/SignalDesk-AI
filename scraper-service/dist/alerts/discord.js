...
async function sendNewLeadsAlert(sourceUrl, platform, posts, batch) {
    ...
    const postEmbeds = leadPosts.map((p) => {
    ...
    const sizeOf = (e) => {
        let n = 0;
        const add = (s) => { if (typeof s === "string")
            n += s.length; };
        add(e.title);
        add(e.description);
        add(e.footer?.text);
    ...
    try {
        ...
        console.log(`[discord] New leads alert sent: ${batch.inserted} ${platform} leads across ${messages.length} message(s)`);
    }
    catch (err) {
