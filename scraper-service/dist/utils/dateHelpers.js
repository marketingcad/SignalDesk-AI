...
exports.isOlderThanCurrentWeek = isOlderThanCurrentWeek;
...
function isCurrentWeek(ts) {
    if (!ts)
        return true; // no date found → keep post
    const d = new Date(ts);
    if (isNaN(d.getTime()))
 ...
 */
function isOlderThanCurrentWeek(ts) {
    if (!ts)
        return false; // can't determine → don't trigger early stop
    const d = new Date(ts);
    if (isNaN(d.getTime()))
        return false;
    return d < getStartOfWeek();
 ...
 */
function resolveTimestamp(rawTs) {
    if (!rawTs)
        return "";
    if (/^\d{4}-/.test(rawTs))
        return rawTs;
